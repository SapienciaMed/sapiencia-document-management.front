...
const DocumentsReceived = () => {
	...
	const getRadicadoIncompleto = () => {
		get(
			`/radicado-details/find-by-create-by/${authorization.user.numberDocument}?tipo=Recibido`
		).then((data: any) => {
	...
	useBreadCrumb({
		...
		url: "/gestion-documental/radicacion/documento-recibido",
	});
		...
		if (data?.add_recipient_data?.length > 0) {
			data.add_recipient_data.forEach((recipient, index) => {
				formData.append(
					...
					recipient.USR_NUMERO_DOCUMENTO || ""
				);
			});
		}
